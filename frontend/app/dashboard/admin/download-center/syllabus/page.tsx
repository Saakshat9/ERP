...
export default function Syllabus() {
    ...
    const fetchSyllabus = async () => {
        try {
            const res = await apiFetch(`${API_ENDPOINTS.STUDY_MATERIAL}?type=syllabus`)
            if (res.ok) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await apiFetch(API_ENDPOINTS.STUDY_MATERIAL, {
                method: "POST",
                body: JSON.stringify({
                    title: form.title, // Map Class/Section to Title or separate logic? using Title as Label
                        ...
                        <CardContent className="pt-6">
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div className="space-y-2">
                                    <Label className="text-red-500">Title / Desc *</Label>
                                    <Input
                                        value={form.title}
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {loading ? (
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={4} className="text-center">No syllabus found.</TableCell>
                                            </TableRow>
                                        ) : (
                                            rows.map((row) => (
